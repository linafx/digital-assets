...
private[lf] object Compiler {
  ...
  def compilePackages(
    ...
    val compiler = Compiler(packages)
    try {
      Right(
...
private[lf] final case class Compiler(packages: PackageId PartialFunction Package) {
  ...
  def unsafeCompileDefn(
    ...
    defn match {
      case DValue(_, _, body, _) =>
        List(LfDefRef(identifier) -> unsafeCompile(body))
  ...
  def closureConvert(remaps: Map[Int, Int], bound: Int, expr: SExpr): SExpr = {
    ...
    expr match {
      ...
      case x: KFun => throw CompilationError(s"unexpected expression: $x")
      case x: KPAP => throw CompilationError(s"unexpected expression: $x")
      case x: KArg => throw CompilationError(s"unexpected expression: $x")
      case x: KFinished => throw CompilationError(s"unexpected expression: $x")
      case x: KMatch => throw CompilationError(s"unexpected expression: $x")
      case x: KCacheVal => throw CompilationError(s"unexpected expression: $x")
      case x: KPop => throw CompilationError(s"unexpected expression: $x")
      case x: KLocationTrace => throw CompilationError(s"unexpected expression: $x")
      case x: KCatchMarker => throw CompilationError(s"unexpected expression: $x")
      case x: KCollectArg => throw CompilationError(s"unexpected expression: $x")
    }
  }
