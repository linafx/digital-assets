...
If you don't want to enter `dev-env` manually each time, you can also install [direnv](https://direnv.net), which will load it for you when you enter the repository directory. This repo already provides a `.envrc` file, with an option to add more in a `.envrc.private` file.
