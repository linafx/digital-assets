...
defineEarlyCutoff op = addBuiltinRule noLint noIdentity $ \(Q (key, file)) old mode -> do
    ...
    case val of
        ...
        Nothing -> do
            ...
            liftIO $ putStrLn $ "ran: " <> show (key, file, fst res)
...
updateFileDiagnostics afp previousAll currentAll = do
    ...
    liftIO $ putStrLn $ "updateFileDiagnostics: " <> show (afp, map (view dFilePath) currentAll, current, map (view dFilePath) previousAll, previous)
