...
class KeyHasherSpec extends AnyWordSpec with Matchers {
  ...
  "KeyHasher.putValue" should {
    ...
    "stable " in {
      ...
      val enums =
        List[Value](
          ValueEnum(Some(EnumTypeCon), EnumCon1, None),
          ValueEnum(Some(EnumTypeCon), EnumCon2, None),
          ValueEnum(Some(EnumTypeConBis), EnumCon2, None),
        )
      ...
      val variants = List[Value](
        ValueVariant(Some(VariantTypeCon), VariantCon1, None, ValueFalse),
        ValueVariant(Some(VariantTypeCon), VariantCon1, None, ValueTrue),
        ValueVariant(Some(VariantTypeCon), VariantCon2, None, ValueFalse),
        ValueVariant(Some(VariantTypeConBis), VariantCon1, None, ValueFalse),
      )
